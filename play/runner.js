        ...
        let importObject = {
            ...
            env: {
                ...
                __tinygo_gpio_set: (pin, high) =>
                    this.mcu.getPin(pin).set(high ? true : false),
                __tinygo_gpio_get: (pin, high) =>
                    this.mcu.getPin(pin).get(),
                __tinygo_gpio_configure: (pin, mode) =>
                    this.mcu.getPin(pin).setMode({
                        0: 'input',
                        1: 'output',
                    }[mode]),
                ...
                __tinygo_ws2812_write_byte: (pinNumber, c) => {
                    for (let pin of this.mcu.getPin(pinNumber).connected) {
                        pin.device.writeWS2812Byte(pin, c);
                    }
                },
            },
        };
        if (file instanceof Response) {
            this.onload(await WebAssembly.instantiateStreaming(file, importObject));
        } else {
            this.onload(await WebAssembly.instantiate(file, importObject));
        }
    }
